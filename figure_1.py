...
axs[1].set_xlabel('BaU')
axs[1].set_ylabel("Percent of Global Population Malnourished in 2040") #This is the only bar chart that gets a y-axis. It will be uniform for the following three bar charts.
axs[1].set_ylim(top=3.5) #Change this if the bars have way too much whitespace above, or don't fit. New data may cause that.
axs[1].yaxis.set_major_formatter(mtick.PercentFormatter(decimals=1))
axs[3].set_title("Percent of Global Population Undernourised in 2040 \nby Scenario and Percentile Target") #This title could go further left, or right, depending on what lets it fit + not be over the line graph
    ...
    if scenario != "BaU": #Skip BaU, it's special.
        ...
        for percentile in target_percentiles:
            ...
            for i in range(0, len(target_years)):
                ...
                axs[iterations+2].scatter(percentile_iterations,value['2040'],marker=symbols[i], color="gray", s = 60,edgecolors='black')
            percentile_iterations += 1
        
        #Give the figure its scenario data (name, scenario average)
        axs[iterations+2].set_xlabel(scenario)
        axs[iterations+2].axhline(y=(sum(values)/len(values)), color="black", linestyle="--")
